...
void				sierpinski_r(t_mlx *mlx, t_triangle tri, int i, bool first)
{
	...
	sierpinski_helper(t, mlx, i, first);
}
...
void				sierpinski_init(t_mlx *mlx)
{
	...
	tri.child[0] = (t_vect_2){ ((tri.parent[0].x + tri.parent[1].x) *
		0.5f), ((tri.parent[0].y + tri.parent[1].y) * 0.5f) };
	tri.child[1] = (t_vect_2){ ((tri.parent[1].x + tri.parent[2].x) *
		0.5f), ((tri.parent[1].y + tri.parent[2].y) * 0.5f) };
	tri.child[2] = (t_vect_2){ ((tri.parent[2].x + tri.parent[0].x) *
		0.5f), ((tri.parent[2].y + tri.parent[0].y) * 0.5f) };
